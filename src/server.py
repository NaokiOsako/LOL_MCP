...
async def get_top_champions(puuid: str, champ_map: dict[int, str], count: int = 3) -> str:
    ...
    for c in mastery_data:
        champ_name = champ_map.get(c['championId'], f"ID({c['championId']})")
        result.append(f"- {champ_name}: Level {c['championLevel']}, {c['championPoints']} pts")
    return "\n".join(result)
...
async def get_player_summary(game_name: str, tag_line: str, language: str = "en_US") -> str:
    ...
    """
    print(f"Searching for player: {game_name}#{tag_line}")
    puuid = await get_puuid(game_name, tag_line)
    if not puuid:
        return f"Failed to find player '{game_name}#{tag_line}'. Please check the spelling and make sure the account exists."
...
@mcp.tool()
async def test_player_lookup(game_name: str, tag_line: str) -> str:
    """
    🔍 Test player lookup to debug connection issues.
    
    Simple test function to verify API connectivity and player existence.
    """
    print(f"Testing lookup for: {game_name}#{tag_line}")
    
    # Test API key
    if not RIOT_API_KEY:
        return "❌ RIOT_API_KEY is not set"
    
    print(f"API Key present: {RIOT_API_KEY[:8]}...")
    
    # Test player lookup
    puuid = await get_puuid(game_name, tag_line)
    if puuid:
        return f"✅ Player found! PUUID: {puuid[:8]}..."
    else:
        return f"❌ Player '{game_name}#{tag_line}' not found. Please check spelling and region."
